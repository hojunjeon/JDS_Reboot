...
def check_python_version():
    ...
    if major < 3 or (major == 3 and minor < 8):
        print(
            "[ERROR] Python 3.8+ is required to run Ouroboros. Please upgrade Python."
        )
        sys.exit(1)
...
def setup_environment_variables():
    ...
    if env_file.exists():
        ...
        print(
            ".env template created successfully. Remember to fill in API keys if using LLMs."
        )
    else:
...
def install_local_skills():
    print_step(
        "Provisioning project-local AI skills into global system agent config..."
    )
    ...
    try:
        ...
        for skill_path in local_skills_dir.iterdir():
            if skill_path.is_dir():
                ...
                print(
                    f"Deploying local skill [{skill_path.name}] to system config: {target_skill_dir}"
                )
        ...
        print(
            "Please manually copy '.ai-workspace/skills/*' to '~/.gemini/config/skills/' if devlog auto-logging fails."
        )
...
def verify_test_suite():
    ...
    try:
        ...
        if result.returncode == 0:
            print(
                "[SUCCESS] Portability test suite passed successfully on this machine!"
            )
        else:
        ...
        print(
            f"[WARN] Failed to execute test suite automatically: {e}. Please run 'pytest' manually."
        )
