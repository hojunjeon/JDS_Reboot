...
class StepState(BaseModel):
    ...
    timestamp: float = Field(default_factory=time.time, description="Unix timestamp of the snapshot")
    files: List[FileState] = Field(default_factory=list, description="State snapshots of all tracked files")
    metric_value: Optional[float] = Field(default=None, description="Progress or performance score metric at this step")
...
class StagnationReport(BaseModel):
    ...
    spinning_files: List[str] = Field(default_factory=list, description="Files stuck in identical repeating hashes")
    oscillation_detected: bool = Field(description="True if OSCILLATION pattern is found")
    oscillation_files: List[str] = Field(default_factory=list, description="Files repeating alternating change cycles")
    nodrift_detected: bool = Field(description="True if progress metrics have flatlined")
    drift_range: Optional[float] = Field(default=None, description="Maximum variance of metrics over the window")
    stagnation_detected: bool = Field(description="True if any stagnation pattern is detected")


class ResilienceAdvice(BaseModel):
    """Actionable lateral thinking advice for rotating developer personas."""
    stagnation_report: StagnationReport = Field(description="The underlying stagnation analysis report")
    recommended_persona: str = Field(description="Hacker, Simplifier, Architect, or Contrarian")
    persona_description: str = Field(description="Core philosophy and operational goals of this persona")
    explanation: str = Field(description="Analytical justification for recommending this rotation")
    action_items: List[str] = Field(default_factory=list, description="Specific tactical code adjustments")
    lateral_thinking_prompt: str = Field(description="Reframing question to disrupt structural deadlocks")
...
class StagnationDetector:
    ...
    @staticmethod
    def detect_spinning(history: List[StepState], threshold: int = 3) -> Tuple[bool, List[str]]:
    ...
    @staticmethod
    def detect_oscillation(history: List[StepState], cycles: int = 2) -> Tuple[bool, List[str]]:
    ...
    @staticmethod
    def detect_no_drift(history: List[StepState], threshold_steps: int = 3, delta: float = 0.01) -> Tuple[bool, Optional[float]]:
        ...
        metrics = [step.metric_value for step in recent_steps if step.metric_value is not None]
    ...
    def advise(self, report: StagnationReport) -> ResilienceAdvice:
        ...
        if not report.stagnation_detected:
            return ResilienceAdvice(
                ...
                action_items=["Maintain current progress velocity.", "Continue writing unit tests."],
...
class WorkspaceTracker:
    ...
    def snapshot_workspace(self, directory: str, step_id: int, metric_value: Optional[float] = None) -> StepState:
        ...
        for root, _, files in os.walk(directory):
            ...
            if any(p in root for p in [".git", "__pycache__", "venv", ".venv", "env", "egg-info", ".antigravitycli"]):
                continue
...
def record(
    ...
    metric: Optional[float] = typer.Option(None, "--metric", "-m", help="Current performance or progress metric value")
):
