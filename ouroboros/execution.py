...
class DoubleDiamondPlanner:
    ...
    def plan(self) -> Tuple[List[List[AcceptanceCriteria]], bool]:
        ...
        if sorted_count < len(criteria):
            ...
            logger.warning(
                "Dependency cycle detected in acceptance criteria! Falling back to sequential execution."
            )
        else:
            self.has_cycle = False
    ...
    def generate_plan_markdown(self) -> str:
        ...
        if not self.spec:
            raise ValueError(
                "Specification is not loaded or planned. Call plan() first."
            )
        ...
        md.append(
            f"- **Concurrency Factor (Avg Tasks/Level):** {concurrency_factor:.2f}"
        )

        if self.has_cycle:
            md.append(
                "- **Execution Mode:** Sequential (Fallback due to cycle detection)"
            )
            md.append("")
            md.append("> [!WARNING]")
            md.append("> **Dependency Cycle Detected!**")
            md.append(
                "> Ouroboros has detected a circular dependency in the acceptance criteria tree."
            )
            md.append(
                "> The engine has gracefully fallen back to sequential execution. Please inspect your acceptance criteria dependencies in `ouroboros_seed.yaml` to unlock parallel level execution."
            )
        else:
        ...
        for phase in phases:
            ...
            for lvl_num, ac in phase_tasks:
                deps_str = (
                    ", ".join(f"AC{d}" for d in ac.depends_on)
                    if ac.depends_on
                    else "None"
                )
        ...
        for lvl_idx, level in enumerate(self.levels):
            ...
            if self.has_cycle:
                ...
                md.append(
                    f"*(Parallel execution enabled: **{concurrency_count}** concurrent tasks)*"
                )
            md.append("")
            ...
            for ac in level:
                ...
                deps_str = (
                    f" (requires {', '.join(f'AC{d}' for d in ac.depends_on)})"
                    if ac.depends_on
                    else ""
                )
                md.append(
                    f"- [ ] **`AC{ac.id}`** [{phase}]: {ac.description}{deps_str}"
                )
            md.append("")
