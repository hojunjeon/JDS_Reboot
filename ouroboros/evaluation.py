...
class MechanicalEvaluationResult(BaseModel):
    ...
    compile_success: bool = Field(description="True if all checked files compiled successfully")
    compile_details: Dict[str, str] = Field(default_factory=dict, description="Detailed status/error per file")
    tests_passed: Optional[bool] = Field(default=None, description="True if unit tests ran and passed")
    tests_stdout: Optional[str] = Field(default=None, description="Standard output from test runner")
    tests_stderr: Optional[str] = Field(default=None, description="Standard error from test runner")
    tests_exit_code: Optional[int] = Field(default=None, description="Exit code from test runner")
    lint_passed: Optional[bool] = Field(default=None, description="True if lint check passed")
    lint_output: Optional[str] = Field(default=None, description="Output from linter check")
    coverage_percentage: Optional[float] = Field(default=None, description="Parsed coverage percentage if available")
...
class RequirementStatus(BaseModel):
    ...
    confidence: float = Field(description="Confidence rating of evaluation between 0.0 and 1.0")
...
class SemanticEvaluationResult(BaseModel):
    ...
    seed_file_path: Optional[str] = Field(default=None, description="Path to seed yaml file used")
    reviewed_files: List[str] = Field(default_factory=list, description="List of files reviewed")
    requirements_statuses: List[RequirementStatus] = Field(default_factory=list, description="Status list for requirements")
    overall_score: float = Field(description="Semantic compliance score between 0.0 and 100.0")
...
class ConsensusReport(BaseModel):
    ...
    consensus_score: float = Field(description="Aggregate consensus score between 0.0 and 100.0")
    ...
    agreement_rate: float = Field(description="Percentage of requirements where all judges agree (0-100)")
    ambiguity_index: float = Field(description="Normalized Ambiguity Index (0.0 to 1.0, where 0.0 is perfect agreement)")
    satisfied_requirements: List[str] = Field(default_factory=list, description="Requirements agreed to be satisfied")
    unsatisfied_requirements: List[str] = Field(default_factory=list, description="Requirements agreed to be unsatisfied")
    conflicts: List[Dict[str, Any]] = Field(default_factory=list, description="List of requirements with conflicting votes")
...
class MechanicalEvaluator:
    ...
    @staticmethod
    def run_tests(test_path: str = "tests", cwd: Optional[str] = None) -> Tuple[bool, Optional[str], Optional[str], int, Optional[float]]:
    ...
    @staticmethod
    def run_linter(file_paths: List[str], cwd: Optional[str] = None) -> Tuple[bool, str]:
        ...
        try:
            res = subprocess.run(black_cmd, capture_output=True, text=True, cwd=cwd, check=False)
            if res.returncode == 0:
            ...
            try:
                res = subprocess.run(flake_cmd, capture_output=True, text=True, cwd=cwd, check=False)
                if res.returncode == 0:
                ...
                for path in file_paths:
                    ...
                    try:
                        ...
                        for i, line in enumerate(lines, 1):
                            ...
                            if line.rstrip("\n") != line.rstrip():
                                violations.append(f"{path}:{i}: Trailing whitespace detected.")
                            # Rule 2: Line too long (PEP 8 max 120 here for flexibility)
                            if len(line) > 120:
                                violations.append(f"{path}:{i}: Line too long ({len(line)} > 120 chars).")
                    except Exception as e:
                        violations.append(f"{path}: Failed to inspect style: {str(e)}")
                if not violations:
                    return True, "AST-based stylistic fallback checks passed successfully (Black/Flake8 not installed)."
                return False, "AST-based stylistic violations found:\n" + "\n".join(violations[:20])

    def evaluate(self, file_paths: List[str], test_path: str = "tests", cwd: Optional[str] = None) -> MechanicalEvaluationResult:
        ...
        tests_passed, stdout, stderr, exit_code, coverage = (None, None, None, None, None)
        if compile_ok and os.path.exists(os.path.join(cwd or "", test_path)):
            tests_passed, stdout, stderr, exit_code, coverage = self.run_tests(test_path, cwd)

        lint_passed, lint_out = self.run_linter(file_paths, cwd)
        
        overall = compile_ok and (tests_passed is not False) and (lint_passed is not False)
...
class SemanticEvaluator:
    ...
    def _load_seed_requirements(self) -> List[Dict[str, Any]]:
        ...
        if self.seed_path and os.path.exists(self.seed_path):
            try:
                ...
                console.print(f"[yellow]Warning: Failed to load seed file {self.seed_path}: {e}. Using defaults.[/yellow]")
    ...
    def _call_llm(self, prompt: str, system_prompt: str) -> Optional[str]:
        ...
        try:
            ...
            console.print(f"[red]LLM API call failed: {e}. Falling back to Regex Checklist.[/red]")
            return None

    def _regex_check_file(self, requirement: Dict[str, Any], file_contents: Dict[str, str]) -> RequirementStatus:
        ...
        if "type" in name or "type" in desc:
            ...
            annotated_def_count = len(re.findall(r"def\s+\w+\(.*?.*?\) -> [A-Za-z_\[\]\s,]+:", all_code, re.DOTALL))
            
            if def_count == 0:
                satisfied = True
                confidence = 0.5
                reasoning = "No Python function definitions found to verify type-checking annotations."
            elif annotated_def_count / def_count >= 0.8 or "List[" in all_code or "Dict[" in all_code or "Optional[" in all_code:
                satisfied = True
                confidence = 0.85
            ...
            for filepath, content in file_contents.items():
                for idx, line in enumerate(content.splitlines(), 1):
                    ...
                    if clean_line in ("pass", "...", '"""TODO"""', "raise NotImplementedError"):
            ...
            if has_todo or placeholder_lines:
                ...
                if placeholder_lines:
                    reasons.append(f"Standard placeholders found at: {', '.join(placeholder_lines[:3])}")
            ...
            has_spinning = "SPINNING" in all_code or "spinning" in all_code or "hash" in all_code
            has_oscillation = "OSCILLATION" in all_code or "oscillation" in all_code or "cycle" in all_code
            has_nodrift = "NO_DRIFT" in all_code or "drift" in all_code
            
            checks = []
            if has_spinning: checks.append("SPINNING hash repeat")
            if has_oscillation: checks.append("OSCILLATION alternating")
            if has_nodrift: checks.append("NO_DRIFT scale delta")
            ...
            words = [w for w in re.findall(r"\b\w{4,}\b", desc) if w not in ("must", "should", "ensure", "verify", "requirements", "satisfy")]
    ...
    def evaluate(self, file_paths: List[str]) -> SemanticEvaluationResult:
        ...
        for path in file_paths:
            if os.path.exists(path):
                try:
                    ...
                    console.print(f"[yellow]Skipping file read for {path}: {e}[/yellow]")
        ...
        prompt_data = {
            ...
            "codebase": {path: content[:20000] for path, content in file_contents.items()}  # truncate to prevent token issues
        }
        ...
        if llm_response:
            try:
                ...
                console.print(f"[yellow]Failed to parse LLM evaluation JSON response: {e}. Falling back to Regex Checklist.[/yellow]")
                statuses = []
...
class ConsensusBuilder:
    ...
    @staticmethod
    def calculate_ambiguity_index(votes_matrix: List[List[bool]]) -> Tuple[float, float]:
    ...
    def build_report(
        ...
        for req_name in all_req_names:
            ...
            for judge_idx, res in enumerate(semantic_results):
                ...
                status_item = next((s for s in res.requirements_statuses if s.requirement == req_name), None)
                vote = status_item.satisfied if status_item else False
                req_votes.append(vote)
                
                judge_name = f"Judge-{judge_idx} ({res.evaluator_type})"
                reason_text = status_item.reasoning if status_item else "No evaluation provided."
                reasons.append(f"- **{judge_name}**: {'[SATISFIED]' if vote else '[FAILED]'} {reason_text}")
        ...
        # Calculate scores
        avg_semantic_score = sum(r.overall_score for r in semantic_results) / total_judges
        
        # Absolute Veto: If mechanical fails, we penalize the overall consensus score heavily
        mechanical_ok = mechanical_result.overall_success
        consensus_score = avg_semantic_score if mechanical_ok else avg_semantic_score * 0.4
        ...
        markdown.append(f"- **Consensus Score**: `{consensus_score:.1f}/100.0` (Veto factored: `{'No' if mechanical_ok else 'Yes'}`)")
        markdown.append(f"- **Mechanical Validation**: `{'PASS' if mechanical_ok else 'FAIL'}`")
        markdown.append(f"- **Semantic Consensus**: `{'PASS' if semantic_ok else 'FAIL'}`")
        markdown.append(f"- **Agreement Rate**: `{agreement_rate:.1f}%` ({len(satisfied_reqs) + len(unsatisfied_reqs) - len(conflicts)}/{len(all_req_names)} in perfect sync)")
        markdown.append(f"- **Ambiguity Index**: `{ambiguity_index:.3f}` (Scale: 0.0 = perfect agreement, 1.0 = total disagreement)\n")

        # Mechanical Details Section
        markdown.append("## ⚙️ Stage 1: Mechanical Health Report\n")
        markdown.append(f"- **Compilation Status**: `{'CLEAN' if mechanical_result.compile_success else 'ERRORS'}`")
        ...
        markdown.append(f"- **Unit Tests**: `{'PASSED' if mechanical_result.tests_passed else 'FAILED/SKIPPED'}` (Exit Code: `{mechanical_result.tests_exit_code}`)")
        if mechanical_result.coverage_percentage is not None:
            markdown.append(f"- **Code Coverage**: `{mechanical_result.coverage_percentage:.1f}%` Check")
        markdown.append(f"- **Style Check (Linter)**: `{'PASSED' if mechanical_result.lint_passed else 'ISSUES FOUND'}`\n")
        ...
        if conflicts:
            ...
            markdown.append("The semantic judges had conflicting evaluations on the following requirements:")
            for conf in conflicts:
                markdown.append(f"\n### Requirement: {conf['requirement']}")
                markdown.append(f"- **Disagreement Ratio**: `{conf['positive_votes']}/{conf['total_votes']}` positive votes.")
                markdown.append(f"- **Majority Decision**: `{'Satisfied' if conf['majority_verdict'] else 'Unsatisfied'}`")
...
def verify(
    ...
    seed: Optional[str] = typer.Option(None, "--seed", "-s", help="Path to seed requirements yaml file"),
    ...
    console.print("[bold cyan]🤖 Initializing Ouroboros Verification Pipeline...[/bold cyan]")
    
    # 1. Mechanical evaluation
    console.print("[yellow]🔨 Stage 1: Running Mechanical Verification (AST compilation, tests, formatting)...[/yellow]")
    me = MechanicalEvaluator()
    mech_result = me.evaluate(files, test_path=tests)
    
    # 2. Semantic evaluation
    console.print("[yellow]🧠 Stage 2: Performing Semantic Review against seed requirements...[/yellow]")
    ...
    console.print("[yellow]⚖️ Stage 3: Aggregating votes and generating Consensus Report...[/yellow]")
    cb = ConsensusBuilder()
