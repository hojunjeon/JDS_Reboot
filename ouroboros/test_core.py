...
def test_config_fallback():
    ...
    print(response)
    print("-" * 50)
    assert "Socratic" in response or "Double Diamond" in response or "[Fallback Mock Response]" in response
...
def test_seed_serialization():
    ...
    print(f"Number of architecture decisions: {len(loaded_spec.architecture_decisions)}")
    print(f"Number of top-level ACs: {len(loaded_spec.acceptance_criteria_tree)}")
    
    # Assert correctness
    assert loaded_spec.title == spec.title
    assert len(loaded_spec.acceptance_criteria_tree) == len(spec.acceptance_criteria_tree)
