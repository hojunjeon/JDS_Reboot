...
def interview(
    ...
    type_str: Optional[str] = typer.Option(
        "greenfield", "--type", "-t", help="greenfield or brownfield"
    ),
    prompt: Optional[str] = typer.Option(
        None, "--prompt", "-p", help="Initial product vision or idea"
    ),
):
...
def plan(
    seed: str = typer.Option(
        ...
        "--seed",
        "-s",
        help="Path to seed requirements yaml file",
    ),
    output: str = typer.Option(
        "ouroboros_plan.md",
        "--output",
        "-o",
        help="Filename to output the generated Double Diamond plan",
    ),
):
    ...
    if not os.path.exists(seed):
        console.print(
            f"[bold red]Error: Seed specification file not found at '{seed}'.[/bold red]"
        )
        console.print(
            "[yellow]Please run 'python run_ouroboros.py interview' first to generate a seed.[/yellow]"
        )
        raise typer.Exit(code=1)

    console.print(
        f"[bold cyan]💎 Decomposing and Sorting Acceptance Criteria from: {seed}...[/bold cyan]"
    )
    try:
        ...
        console.print(
            Panel(
                Text(
                    f"🎉 Double Diamond execution plan successfully written to:\n[underline]{output}[/underline]\n\n"
...
def evaluate(
    files: Optional[List[str]] = typer.Argument(
        None,
        help="List of Python files to evaluate. Auto-discovers active files if omitted.",
    ),
    seed: str = typer.Option(
        "ouroboros_seed.yaml",
        "--seed",
    ...
    if not files:
        ...
        if not files:
            console.print(
                "[bold red]Error: No Python files found in the current workspace to evaluate.[/bold red]"
            )
            raise typer.Exit(code=1)
    ...
    console.print(
        f"[bold cyan]🔍 Target files discovered for evaluation: {', '.join(normalized_files)}[/bold cyan]"
    )

    # 3. Trigger evaluation pipeline from evaluation.py
    run_evaluation_verify(files=normalized_files, seed=seed, tests=tests, output=output)
...
def status(
    ...
    metric: Optional[float] = typer.Option(
        ...
        "--metric",
        "-m",
        help="Current progress score or performance metric (e.g. consensus score)",
    ),
):
    ...
    if not os.path.exists(directory):
        console.print(
            f"[bold red]Error: Workspace directory '{directory}' does not exist.[/bold red]"
        )
        raise typer.Exit(code=1)

    console.print(
        "[bold cyan]🔄 Recording workspace state snapshot and checking stagnation patterns...[/bold cyan]"
    )
...
def evolve(
    ...
    agents: str = typer.Option(
        "AGENTS.md", "--agents", "-a", help="Path to binding session rules file"
    ),
    ...
    console.print(
        "[bold cyan]🔄 Running diagnostic scans across all development stages...[/bold cyan]"
    )

    try:
        engine = EvolutionEngine(
            seed_path=seed, agents_path=agents, history_path=history
        )
        ...
        console.print(
            f"[yellow]⚠️ Diagnosed {len(failures)} failures. Synthesizing anti-regression patches...[/yellow]"
        )
        patches = engine.generate_patches(failures)

        console.print(
            "[yellow]🔧 Applying permanent spec patches and binding rules to prevent recurrence...[/yellow]"
        )
        ...
        console.print(
            Panel(
                Text(
                    f"🎉 Ouroboros has successfully evolved!\n\n"
        ...
        for idx, p in enumerate(patches, 1):
            console.print(
                f'  [bold]{idx}. Recurrence Prevention Rule:[/bold] [italic]"{p.session_rule}"[/italic]'
            )
            console.print(
                f'     [bold]System Constraint Patched:[/bold] [dim]"{p.constraint}"[/dim]'
            )
