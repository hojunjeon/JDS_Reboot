...
class FailureDiagnosis(BaseModel):
    ...
    source: str = Field(description="Name of the file, test, requirement, or pattern causing failure")
    error_message: str = Field(description="Detailed error trace, stdout, or description of failure")


class EvolutionPatch(BaseModel):
    """Concrete system patches generated to prevent failure recurrence."""
    constraint: str = Field(description="A system constraint to append to ouroboros_seed.yaml")
    session_rule: str = Field(description="An anti-regression rule to append to AGENTS.md")
    explanation: str = Field(description="Analytical justification for why this patch prevents future failure")


class EvolutionEngine:
    """Core engine driving Phase 5: Evolution and Anti-Regression Patching."""

    def __init__(self, seed_path: str = "ouroboros_seed.yaml", agents_path: str = "AGENTS.md", history_path: str = ".ouroboros_history.json"):
        self.seed_path = seed_path
        self.agents_path = agents_path
        self.history_path = history_path

    def diagnose_workspace(self, files: List[str], test_path: str = "tests") -> List[FailureDiagnosis]:
        ...
        if mech_res.tests_passed is False:
            failures.append(FailureDiagnosis(
                category="MECHANICAL",
                source="pytest suite",
                error_message=mech_res.tests_stderr or mech_res.tests_stdout or "Unit test execution failed."
            ))
        ...
        if mech_res.lint_passed is False:
            failures.append(FailureDiagnosis(
                ...
                error_message=mech_res.lint_output or "Style compliance check failed."
            ))
        ...
        if os.path.exists(self.history_path):
            ...
            if report.stagnation_detected:
                ...
                if report.spinning_detected:
                    patterns.append(f"SPINNING (identical file repeating: {', '.join(report.spinning_files)})")
                if report.oscillation_detected:
                    patterns.append(f"OSCILLATION (alternating cycles: {', '.join(report.oscillation_files)})")
                if report.nodrift_detected:
                    patterns.append(f"NO_DRIFT (metric flatline with drift range {report.drift_range})")
    ...
    def generate_patches(self, failures: List[FailureDiagnosis]) -> List[EvolutionPatch]:
        ...
        if openai_key or gemini_key or anthropic_key:
            ...
            if llm_response:
                try:
                    ...
                    print(f"[Evolution Engine Warning] LLM parse failed: {e}. Falling back to Rule-Based templates.")
                    patches = []
    ...
    def generate_report(self, failures: List[FailureDiagnosis], patches: List[EvolutionPatch], yaml_patched: int, agents_patched: int) -> str:
        ...
        markdown.append(f"Found **{len(failures)}** active failure patterns across current development stages:\n")
        ...
        markdown.append(f"Successfully generated **{len(patches)}** permanent system modifications:\n")
        ...
        markdown.append(f"- `ouroboros_seed.yaml`: Added **{yaml_patched}** new constraints.")
        markdown.append(f"- `AGENTS.md`: Added **{agents_patched}** new anti-regression rules.\n")
