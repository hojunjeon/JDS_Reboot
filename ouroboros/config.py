...
"""
    ...
    elif "double diamond" in prompt_lower or "planning" in prompt_lower or "mvp" in prompt_lower:
...
def call_llm(
    ...
    if not chosen_provider:
        if GEMINI_API_KEY:
            ...
            console.log("[yellow]No API keys found in environment. Using robust rules/mock fallback.[/yellow]")
    ...
    try:
        if chosen_provider == "gemini":
            ...
            return _call_gemini(prompt, system_prompt, target_model, temp, GEMINI_API_KEY)
            ...
            return _call_openai(prompt, system_prompt, target_model, temp, OPENAI_API_KEY)
            ...
            return _call_anthropic(prompt, system_prompt, target_model, temp, ANTHROPIC_API_KEY)
            
        else:
            raise ValueError(f"Unknown or unsupported LLM provider: {provider}")
            
    except Exception as e:
        console.log(f"[red]LLM invocation failed: {e}. Falling back gracefully to mocks...[/red]")
        return generate_fallback_response(prompt, system_prompt)
