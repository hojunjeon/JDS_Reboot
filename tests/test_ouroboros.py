...
def test_seed_spec_serialization(tmp_path):
    ...
    criteria = [
        AcceptanceCriteria(
            id=100, description="Discover phase requirements", depends_on=[]
        ),
        ...
        AcceptanceCriteria(
            id=103, description="Implement code and evaluate", depends_on=[102]
        ),
    ]
    spec = SeedSpec(
...
def test_topological_sort_no_cycle(tmp_path):
    ...
    criteria = [
        AcceptanceCriteria(
            id=100, description="Research and Socratic interview", depends_on=[]
        ),
        ...
        AcceptanceCriteria(
            id=400, description="Implement and test", depends_on=[200, 300]
        ),
    ]
    spec = SeedSpec(
        title="Test DAG", description="Test Desc", acceptance_criteria_tree=criteria
    )
    save_to_yaml(spec, yaml_file)
...
def test_topological_sort_with_cycle(tmp_path):
    ...
    spec = SeedSpec(
        title="Test Cycle", description="Test Desc", acceptance_criteria_tree=criteria
    )
    save_to_yaml(spec, yaml_file)
...
def test_phase_classification():
    ...
    ac_discover = AcceptanceCriteria(
        id=100, description="Conduct socratic interview and clarify ambiguity"
    )
    assert classify_ac_phase(ac_discover, 4, 3) == "Discover"

    ac_define = AcceptanceCriteria(
        id=200, description="Establish criteria limits and system boundaries"
    )
    assert classify_ac_phase(ac_define, 4, 3) == "Define"

    ac_design = AcceptanceCriteria(
        id=300, description="Draw structural layout and database design blueprints"
    )
    assert classify_ac_phase(ac_design, 4, 0) == "Design"

    ac_deliver = AcceptanceCriteria(
        id=400, description="Write final python cli execution and verify compliance"
    )
...
def test_generate_plan_markdown(tmp_path):
    ...
    criteria = [
        ...
        AcceptanceCriteria(
            id=400, description="Implement and deploy", depends_on=[300]
        ),
    ]
    spec = SeedSpec(
...
def test_mechanical_syntax_checking(tmp_path):
    ...
    success, details = MechanicalEvaluator.check_syntax(
...
def test_semantic_regex_checklist():
    ...
    type_req = {
        "name": "Type Safety",
        "description": "All python code must be fully-typed with return types and parameters annotated",
        "pattern": "type_safety",
    }
    ...
    placeholder_req = {
        ...
        "description": "No placeholders, empty ellipses (...), or TODO comments",
        "pattern": "no_placeholders",
    }
    placeholder_status = se._regex_check_file(
        placeholder_req, {"main.py": "def test():\n    pass\n"}
    )
    assert placeholder_status.satisfied is False

    pydantic_req = {
        "name": "Pydantic usage",
        "description": "Use pydantic BaseModel class and Field definitions",
...
def test_consensus_ambiguity_calculation():
    ...
    votes_matrix = [[True, True], [True, False]]

    agreement_rate, ambiguity_index = ConsensusBuilder.calculate_ambiguity_index(
        votes_matrix
    )
    ...
    judge_res = [
        SemanticEvaluationResult(
            ...
            requirements_statuses=[
                RequirementStatus(
                    requirement="Req 1", satisfied=True, confidence=1.0, reasoning="OK"
                )
            ],
            overall_score=100.0,
            passed=True,
...
def test_stagnation_no_drift():
    ...
    stalled, drift = StagnationDetector.detect_no_drift(
        history_stalled, threshold_steps=3, delta=0.01
    )
    ...
    stalled_2, drift_2 = StagnationDetector.detect_no_drift(
        history_drifting, threshold_steps=3, delta=0.01
    )
